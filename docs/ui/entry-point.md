...
The Entry Point owns the runtime theme: it creates the kit's UI-local shared-state store (`createSharedState`), hands it to the shell (which reflects `theme` as `data-theme`), reads the stored theme once at startup, and re-applies it on every `settings:changed` domain event (serialized through a write chain so rapid saves converge in order). The stored `device` value is normalized to the kit vocabulary `system` before reflecting — otherwise it would match no stylesheet scope while the editor independently resolves dark, leaving light text on a light page. Each apply writes the normalized value to both the store and `document.documentElement.dataset.theme` — the document scope carries the dark token overrides in `public/theme.css`, which head-injected editor styles can also resolve. See `../decisions.md` §12.
