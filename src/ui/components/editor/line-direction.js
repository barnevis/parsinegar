...
// the same explicit treatment: the first strong letter of each line decides
// its direction (mirroring the `plaintext` heuristic), and a line
// decoration pins that direction together with a matching explicit
// alignment. No stylesheet `plaintext` remains on `.cm-line`.
 ...
 * direction: lettered lines to their own script direction, letter-less
 * lines to the base direction.
 ...
 */
function buildDirectionDecorations(view, baseDirection) {
  const builder = [];
  ...
  for (const { from, to } of ranges) {
    for (let pos = from; pos <= to;) {
      ...
      const resolved = resolveLineDirection(line.text);
      const lineClass = resolved === 'ltr'
        ? 'parsi-dir-ltr'
        : resolved === 'rtl'
          ? 'parsi-dir-rtl'
          : baseClass;
...
export function lineDirectionExtensions(baseDirection) {
  const plugin = ViewPlugin.fromClass(
    class {
      constructor(view) {
        this.decorations = buildDirectionDecorations(view, baseDirection);
      }

      update(update) {
        if (update.docChanged || update.viewportChanged) {
          this.decorations = buildDirectionDecorations(update.view, baseDirection);
        }
      }
    },
