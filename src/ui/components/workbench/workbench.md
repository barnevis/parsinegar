...
- `views-files.js` — `renderFilesView({ t, items, currentId, openMenuId, editing, assetBaseUrl, sortMode })`: document list with management actions; each row carries a menu button opening the per-file menu (rename, download, properties, delete), and the editing row renders an inline rename input with an optional error. The header carries a sort select (`data-files-sort`); `sortDocuments(items, mode)` orders a copy by update/creation date (either direction) or Persian-alphabetical name (`FILES_SORT_MODES`, default `updated-desc`).
