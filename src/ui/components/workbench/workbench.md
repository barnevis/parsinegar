...
- `views-files.js` — `renderFilesView({ t, items, currentId, openMenuId, editing, assetBaseUrl, sortMode, sortMenuOpen })`: document list with management actions; each row carries a menu button opening the per-file menu (rename, download, properties, delete), and the editing row renders an inline rename input with an optional error. The header carries bare icon buttons — new-document (Add Notes), import (Upload, emits `document-import`) and sort — plus a grouped sort menu like the file menus (name: alphabetical/reverse; update/creation date: newest/oldest); the active option is distinguished with the accent wash and bold, and the menu type is smaller (`0.85rem`); `sortDocuments(items, mode)` orders a copy accordingly (`FILES_SORT_MODES`, `FILES_SORT_GROUPS`, `FILES_SORT_LABELS`, default `updated-desc`).
- `views-outline.js` — `renderOutlineView({ t, documentText, activeLine, collapsed })`: heading outline with navigation targets; the heading on `activeLine` renders with `aria-current` (scrollspy highlight), and headings in `collapsed` hide their children behind a toggle button.
...
- `menu-model.js` — `buildMenuModel({ t, hasDocument })`: menu bar as pure data consumed by `parsi-menu-bar` (file menu: new, import, delete, about, github).
